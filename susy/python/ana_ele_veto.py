...
#----------------------
    ...
    def Fun_tight_ele_antiIso(ind,pfiso):
        if abs(tree.eleSCEta[ind])<=1.479 and pfiso>0.175 and pfiso<1. and tree.eledEtaseedAtVtx[ind]<0.00308 and tree.eledPhiAtVtx[ind]<0.0816 and tree.eleHoverE[ind]<0.0414 and tree.eleEoverPInv[ind]<0.0129 and tree.eleConvVeto[ind]==1 and tree.eleSigmaIEtaIEtaFull5x5[ind]<0.00998 and tree.eleMissHits[ind]<=1:
            return True
        elif abs(tree.eleSCEta[ind])>1.479 and pfiso>0.159 and pfiso<1. and tree.eledEtaseedAtVtx[ind]<0.00605  and tree.eledPhiAtVtx[ind]<0.0394 and tree.eleHoverE[ind]<0.0641 and tree.eleEoverPInv[ind]<0.0129 and tree.eleSigmaIEtaIEtaFull5x5[ind]<0.0292 and tree.eleMissHits[ind]<=1 and tree.eleConvVeto[ind]==1: 
            return True
        else: return False
        


    def Fun_veto_ele(ind,pfiso):
        if abs(tree.eleSCEta[ind])<=1.479 and pfiso<0.175 and tree.eledEtaseedAtVtx[ind]<0.00749 and tree.eledPhiAtVtx[ind]<0.228 and tree.eleHoverE[ind]<0.356 and tree.eleEoverPInv[ind]<0.299 and tree.eleConvVeto[ind]==1 and tree.eleSigmaIEtaIEtaFull5x5[ind]<0.0115 and tree.eleMissHits[ind]<=2:
            return True
        elif abs(tree.eleSCEta[ind])>1.479 and pfiso<0.159 and tree.eledEtaseedAtVtx[ind]<0.00895  and tree.eledPhiAtVtx[ind]<0.213 and tree.eleHoverE[ind]<0.211 and tree.eleEoverPInv[ind]<0.15 and tree.eleSigmaIEtaIEtaFull5x5[ind]<0.037 and tree.eleMissHits[ind]<=3 and tree.eleConvVeto[ind]==1: 
            return True
        else: return False
...
#----------------------------
    ...
    for e in range(tree.nEle):
        ...
        if tree.elePt[e]>30 and abs(tree.eleEta[e])<2.1 and Fun_tight_ele(e,PFiso):
            ele=[e,1]
        elif tree.elePt[e]>10 and abs(tree.eleEta[e])<2.5 and Fun_veto_ele(e,PFiso):
            ele=[e,0]
