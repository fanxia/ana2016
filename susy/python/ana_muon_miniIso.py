...
#-----------------
    ...
    for m in range(tree.nMu):
        ...
        muminiIso=tree.muPFMiniIso[m]

        if tree.muPt[m]>30 and abs(tree.muEta[m])<2.1 and muminiIso<=0.1 and tree.muIDbit[m]>>2&1==1:
            mu=[m,1,iso]
        elif  tree.muPt[m]>10 and abs(tree.muEta[m])<2.5 and muminiIso<=0.4 and tree.muIDbit[m]>>0&1==1:
            mu=[m,0,iso]
        elif tree.muPt[m]>30 and abs(tree.muEta[m])<2.1 and muminiIso>0.4 and tree.muIDbit[m]>>2&1==1:
            mu=[m,3,iso]
        else: continue
