...
ScriptName = "calBtagEff_step1.py"
print "****************Begin submit btagEff jobs for ",outputfile
if '.root' in GetIn:
...
#os.system("rm -r log 2>/dev/null")
   os.system("mkdir -p "+sys.argv[2]+"_BtagEff_step1_log")
...
##### loop for creating and sending jobs #####
   ...
   ###### sends bjobs ######
      os.system("bsub -q "+queue+" -o "+sys.argv[2]+"_BtagEff_step1_log/log{0} ".format(x)+sys.argv[2]+"_job{0}.sh".format(x))
      print "job no. " + str(x) + " submitted"
      os.system("cp "+sys.argv[2]+"_job{0}.sh ".format(x)+sys.argv[2]+"_BtagEff_step1_log/")
   #os.chdir("../..")
...
if not '.root' in GetIn:
   tmpinputfilelist=getoutput('ls /eos/cms'+GetIn+'*.root').split('\n')
...
##### loop for creating and sending jobs #####
   ...
   ##### creates jobs #######
      ...
      with open(sys.argv[2]+'_job{0}.sh'.format(x), 'w') as fout:fout.write(jobscr.replace('JOBID',str(x)).replace('INPUT',INPUTthis).replace('OUTNAME',sys.argv[2]))
      os.system("chmod 755 "+sys.argv[2]+"_job{0}.sh".format(x))
   ###### sends bjobs ######
      os.system("bsub -q "+queue+" -o "+sys.argv[2]+"_BtagEff_step1_log/log{0} ".format(x)+sys.argv[2]+"_job{0}.sh".format(x))
